...
export default function MeetingsView({ leads, onSelectLead }: MeetingsViewProps) {
  ...
  const getWeekRangeString = () => {
    ...
    const monStr = monday.toLocaleDateString('en-US', { day: 'numeric', month: 'short' });
    const sunStr = sunday.toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' });
    return `${monStr} – ${sunStr}`;
  };

  const completedMeetingsCount = meetingsList.filter(m => m.status === 'Completed').length;
    ...
    <div className="meetings-layout-grid" style={{ animation: 'fadeUp 0.3s ease', display: 'grid', gridTemplateColumns: '1fr 320px', gap: '16px', alignItems: 'start' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <h3 style={{ margin: '0 0 2px 0', fontSize: '14px', fontWeight: 700, color: '#9AA1AD', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
          Upcoming &middot; {upcomingMeetings.length}
        </h3>
      ...
      <div style={{ background: '#fff', border: '1px solid #ECEDEF', borderRadius: '16px', padding: '18px', height: 'fit-content' }}>
        ...
        <p style={{ margin: '0 0 16px 0', fontSize: '12.5px', color: '#9AA1AD', fontWeight: 500 }}>{getWeekRangeString()}</p>
        
        <div style={{ display: 'flex', flexDirection: 'column', gap: '13px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: '13px', color: '#5A616E', fontWeight: 600 }}>Meetings booked</span>
            <span style={{ fontSize: '16px', fontWeight: 800, color: '#16191D' }}>{upcomingMeetings.length}</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: '13px', color: '#5A616E', fontWeight: 600 }}>Completed</span>
            <span style={{ fontSize: '16px', fontWeight: 800, color: '#16A34A' }}>{completedMeetingsCount}</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: '13px', color: '#5A616E', fontWeight: 600 }}>Cancelled</span>
            <span style={{ fontSize: '16px', fontWeight: 800, color: '#DC2626' }}>{cancelledMeetingsCount}</span>
          </div>
        </div>
